...
local function VisitCities()
    ...
    for step = 1, #cities do
        ...
        yield("/wait 2") 
        yield("/echo Shout in " .. city.name .. " → stinks <se.7>") 
        yield("/wait 2") 
...
-- Normal world transfer with 25-second buffer
local function WaitForTransfer(world)
    ...
    while IPC.Lifestream.IsBusy() and waitTime < maxWait do
        yield("/wait " .. checkInterval)
        waitTime = waitTime + checkInterval

        if waitTime > bufferTime and not Svc.Condition[45] then
            yield("/echo Transfer to " .. world .. " failed (congested). Aborting... <se.6>")
    ...
    if waitTime >= maxWait then
        yield("/echo Transfer to " .. world .. " timed out. Aborting... <se.6>")
        IPC.Lifestream.Abort()
        return false
    end
...
-- Datacenter/world transfer: simplified, maxWait = 200, no [45] checks
local function WaitForDCTravel(world)
    if not IPC or not IPC.Lifestream then
        yield("/echo Error: Lifestream plugin not available! <se.6>")
        return false
    end
...
for i = 1, 3 do
    ...
    if WaitForDCTravel("Ultros") then
        yield("/wait 2")
        yield("/busy")
        VisitCities()
    end
    ...
    if WaitForDCTravel("Zalera") then
        yield("/wait 2")
        yield("/busy")
        VisitCities()
    end
